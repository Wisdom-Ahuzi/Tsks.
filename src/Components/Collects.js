...
const Collects = ({ title, General, Completed }) => {
  ...
  useEffect(() => {
    const q = query(
      collection(db, `${sessionStorage.getItem("currentuserId")}-${Completed}`)
    );
  ...
  useEffect(() => {
    const q = query(
      collection(db, `${sessionStorage.getItem("currentuserId")}-${General}`)
    );
  ...
  const handleReAddedTask = async (com) => {
    await deleteDoc(
      doc(db, `${authentication.currentUser.uid}-${Completed}`, com.id)
    );

    await addDoc(
      collection(db, `${authentication.currentUser.uid}-${General}`),
      {
        text: com.text.text,
        day: day,
      }
  ...
  const handleDone = async (task, id) => {
    await deleteDoc(
      doc(db, `${authentication.currentUser.uid}-${General}`, id)
    );
    ...
    tasks.forEach(async (one) => {
      if (one == task) {
        ...
        await addDoc(
          collection(db, `${authentication.currentUser.uid}-${Completed}`),
          {
            text: one,
            day: day,
          }
  ...
  const handleDeleteTask = async (id) => {
    await deleteDoc(
      doc(db, `${authentication.currentUser.uid}-${Completed}`, id)
    );
  };
      ...
      <div className="schoolDets">
        <span className="schoolHeader">
          <NavLink to="/Collections">
            {" "}
            <img
              src={require("../assets/Desktop/back.png")}
              alt="Dashboard"
              title="Dashboard"
            />{" "}
          </NavLink>
          ...
          />
          <img
            src={require("../assets/Desktop/search.png")}
            alt=""
        ...
        <span>
          <img
            src={require("../assets/Desktop/addTask.png")}
            alt="Add Tasks"
            title="Add Tasks"
            ...
            .map((task) => (
              <div key={uuidv4()} className="tasks">
                <input
                  ...
                  name=""
                  onChange={() => {
                    handleDone(task, task.id);
                  }}
                  id=""
                />
                <p>{task.text}</p>
                <img
                  src={require("../assets/Desktop/calendar.png")}
                  alt="Date Added"
                  title="Date Added"
                />
                <p>{task.day}</p>
            ...
            .map((com) => {
              return (
                <div key={uuidv4()} className="complete">
                  ...
                  <p>{day}</p>
                  <img
                    src={require("../assets/Desktop/trash.png")}
                    alt=""
                    onClick={() => {
                      handleDeleteTask(com.id);
                    }}
                  />
                </div>
              );
