...
const Dashboard = ({
  ...
  useEffect(() => {
    const q = query(
      collection(db, `${sessionStorage.getItem("currentuserId")}-SchoolGeneral`)
    );
  ...
  useEffect(() => {
    const q = query(
      collection(db, `${sessionStorage.getItem("currentuserId")}-DesignGeneral`)
    );
    const unSubscribe = onSnapshot(q, (querySnapshot) => {
      let todoArr = [];
      querySnapshot.forEach((doc) => {
    ...
    <div className="Dashboard" id={extendId}>
      ...
      <div className="innerDashboard">
        <div className="dashboardInfo">
          <h4>Dashboard</h4>
          <h2>
            Hey, <br /> {sessionStorage.getItem("userDisplayName")}
          </h2>
              ...
              <div className="overview" key={uuidv4()}>
                ...
                <div className="overviewLink">
                  <NavLink to={item.to}>
                    ...
                    Go to {item.word}{" "}
                    <img
                      src={require("../assets/Desktop/goto.png")}
                      alt="GoTo"
                      title="GoTo"
                    />{" "}
                  </NavLink>
